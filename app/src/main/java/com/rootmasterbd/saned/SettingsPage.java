...
public class SettingsPage {
    public SettingsPage(Context ctx, View v, ExecutorService exec, Handler handler) {
        ...
        btnKill.setBackgroundColor(Color.parseColor("#FF4060"));

        swAuto.setChecked("1".equals(RootUtils.loadPref("auto_switch","0")));
        swKill.setChecked("1".equals(RootUtils.loadPref("kill_all","0")));
        String wl=RootUtils.loadPref("whitelist","net.jahez.fleets io.suqi8.saned com.google.android.apps.maps com.termux");
        etWl.setText(wl);
        ...
        btnKill.setOnClickListener(x->{
            ...
            exec.execute(()->{
                String wlist=etWl.getText().toString().trim().replace(" ","|");
                RootUtils.su("for pid in $(ls /proc/|grep -E '^[0-9]+$');do pkg=$(cat /proc/$pid/cmdline 2>/dev/null|tr -d '\\0'|cut -d: -f1);echo \"$pkg\"|grep -q '\\.'||continue;echo \"$pkg\"|grep -qE '"+wlist+"|system|zygote|android|com.google.android.gms'&&continue;kill -9 $pid 2>/dev/null;done");
